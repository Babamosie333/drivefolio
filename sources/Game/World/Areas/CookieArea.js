...
export class CookieArea extends Area
{
    ...
    setBlower()
    {
        const blowerRef = this.references.items.get('blower')
        if(blowerRef && blowerRef.length > 0)
            this.blower = blowerRef[0]
        else
            this.blower = null
    }
    ...
    setBanner()
    {
        const bannerRef = this.references.items.get('banner')
        if(!bannerRef || bannerRef.length === 0) return

        const windStrength = float(0).toVarying()
        const mesh = bannerRef[0]
    ...
    setParticles()
    {
        const chimneyRef = this.references.items.get('chimney')
        if(!chimneyRef || chimneyRef.length === 0) return
        ...
        mesh.position.copy(chimneyRef[0].position)
        mesh.count = count
        ...
        this.events.on('frustumIn', () =>
        {
            if(frustumNeedsUpdate)
            {
                ...
                {
                    // Safety check for boundingSphere
    ...
    setOvenHeat()
    {
        const ovenHeatRef = this.references.items.get('ovenHeat')
        if(!ovenHeatRef || ovenHeatRef.length === 0) return
    ...
    setCookies()
    {
        const cookieRef = this.references.items.get('cookie')
        if(!cookieRef || cookieRef.length === 0) return

        const baseCookie = cookieRef[0]
    ...
    setInteractivePoint()
    {
        const interactiveRef = this.references.items.get('interactivePoint')
        if(!interactiveRef || interactiveRef.length === 0) return

        this.game.interactivePoints.create(
            interactiveRef[0].position,
            'Accept cookie',
    ...
    setCounter()
    {
        ...
        const counterPanelRef = this.references.items.get('counterPanel')
        this.counter.panel = counterPanelRef ? counterPanelRef[0] : null
        ...
        this.counter.init = () =>
        {
            ...
            const counterLabelRef = this.references.items.get('counterLabel')
            if(counterLabelRef && counterLabelRef.length > 0)
            {
                const mesh = new THREE.Mesh(geometry, material)
                mesh.position.copy(counterLabelRef[0].position)
                mesh.quaternion.copy(counterLabelRef[0].quaternion)
        ...
        this.counter.update = () =>
        {
            ...
            if(scale > this.counter.maxScale && this.counter.panel)
            {
        ...
        setInterval(() => { this.counter.throttleUpdate() }, 1000)
    ...
    update()
    {
        ...
        if(this.cookies && this.cookies.visibleCount)
        {
