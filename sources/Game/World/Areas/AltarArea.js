...
export class AltarArea extends Area
{
    ...
    setData()
    {
        ...
        this.data.insert = () =>
        {
            this.game.server.send({ type: 'cataclysmInsert' })
        }
    ...
    updateText(value)
    {
        ...
        gsap.to(this.mesh.scale, {
            x: 1.5, y: 1.5, duration: 0.3, overwrite: true,
            onComplete: () =>
            {
                gsap.to(this.mesh.scale, { x: 1, y: 1, duration: 2, ease: 'elastic.out(1,0.3)', overwrite: true })
            }
        })
    }
