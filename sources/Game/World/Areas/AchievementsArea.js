...
import {
  color, cos, float, Fn, instancedArray, instanceIndex, min, mix,
  sin, texture, uniform, uv, varying, vec2, vec3
} from 'three/tsl'
...
export class AchievementsArea extends Area
{
    ...
    setWaterfall()
    {
        ...
        if(!reference || !reference.geometry || !reference.geometry.attributes)
        {
            console.warn('AchievementsArea: waterfallParticles reference not found')
            return
        }
    ...
    setPillar()
    {
        const pillarRef = this.references.items.get('pillar')
        if(!pillarRef || pillarRef.length === 0)
        {
            console.warn('AchievementsArea: pillar reference not found')
            this.pillar = null
            return
        }

        this.pillar = pillarRef[0]
        ...
        mesh.position.set(this.pillar.position.x, 2, this.pillar.position.z)
        mesh.count = count
