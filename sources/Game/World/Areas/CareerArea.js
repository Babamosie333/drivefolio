...
export class CareerArea extends Area
{
    ...
    setLines()
    {
        ...
        // Safety check for line references
        if(!lineGroups || lineGroups.length === 0)
        {
            console.warn('CareerArea: no line references found')
            return
        }
        ...
        for(const group of lineGroups)
        {
            ...
            // Safety check for stone
            if(!line.stone)
            {
                console.warn('CareerArea: stone not found in line group')
                continue
            }
...

            {
                ...
                // Safety check for textMesh
                if(!line.textMesh)
                {
                    console.warn('CareerArea: careerText mesh not found')
                ...
                if(!line.texture)
                {
                    console.warn(`CareerArea: texture not found for ${line.group.userData.texture}`)

                    // Use fallback material without texture
                    material.outputNode = Fn(() =>
                    {
                        const baseUv = uv().toVar()
                        step(baseUv.x, line.labelReveal).lessThan(0.5).discard()
                        const emissiveColor = baseColor.mul(1.7)
    ...
    setYears()
    {
        ...
        // Safety check for year reference
        if(!this.year.group)
        {
            console.warn('CareerArea: year reference not found')
            return
        }
    ...
    {
        // Safety check
        if(!this.lines || !this.lines.items) return

        // Lines
        for(const line of this.lines.items)
        {
            // Safety check for stone
            if(!line.stone) continue
        ...
        // Year - safety check
        if(!this.year || !this.year.group) return
